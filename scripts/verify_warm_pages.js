...
function get(urlPath, headers = {}) {
  return new Promise((resolve) => {
    ...
    const req = http.get(`http://localhost:3000${urlPath}`, { headers, timeout: 15000 }, (res) => {
      let data = '';
...
async function verifyWarmScorecard() {
  console.log('========================================================================');
  console.log('⚡ CUSTOMERPILOT WARM SYSTEM PAGE & API STATUS SCORECARD');
  console.log('========================================================================');
  ...
  const tests = [
    // Public Pages
    { category: '🌐 PUBLIC PAGES', path: '/', name: 'Landing Page & 9-Step Onboarding', headers: {} },
    { category: '🌐 PUBLIC PAGES', path: '/login', name: 'Merchant Login Page', headers: {} },
    { category: '🌐 PUBLIC PAGES', path: '/pricing', name: 'Pricing Matrix & ROI', headers: {} },
    ...
    // Core APIs
    { category: '🔌 CORE APIs', path: '/api/state', name: 'Live Merchant State API', headers: mHeaders },
  ...
  for (const t of tests) {
    ...
    const icon = ok ? '✅' : '❌';
    console.log(`${icon} [HTTP ${res.status}] ${t.name.padEnd(32)} -> ${t.path.padEnd(25)} (${res.ms}ms, ${res.length} bytes)`);
  }
...
verifyWarmScorecard().catch(console.error);

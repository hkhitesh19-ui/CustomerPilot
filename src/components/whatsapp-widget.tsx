    ...
    <div className="fixed bottom-5 right-5 z-50 flex flex-col items-end gap-2 group select-none">
      ...
      {showTooltip && (
        <div className="relative bg-white text-slate-800 text-xs font-semibold py-2 px-3.5 rounded-2xl shadow-xl border border-slate-100 flex items-center gap-2 animate-bounce transition-all duration-300">
          <span>Need Help? <strong>Chat on WhatsApp</strong></span>
          <button
            onClick={(e) => {
              e.stopPropagation()
      ...
      <a
        ...
        aria-label="Chat with CustomerPilot on WhatsApp"
        className="relative flex items-center justify-center w-14 h-14 bg-[#25D366] hover:bg-[#20bd5a] text-white rounded-full shadow-lg shadow-emerald-500/30 hover:shadow-emerald-500/50 hover:scale-110 active:scale-95 transition-all duration-300 focus:outline-hidden"
      >
